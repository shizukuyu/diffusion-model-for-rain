...
class DDPM(BaseModel):
    ...
    def __init__(self, in_channel, out_channel, norm_groups, inner_channel,
        ...
        self.resume_state = resume_state if resume_state else ""
    ...
    def load_network(self) -> None:
        ...
        if not self.resume_state:  # 如果resume_state为空或None，直接返回
            logger.info("No pretrained model specified, skipping loading.")
            return
            
        gen_path = f"{self.resume_state}_gen.pth"
        opt_path = f"{self.resume_state}_opt.pth"
        
        # 检查文件是否存在
        if not os.path.exists(gen_path):
            logger.warning(f"Pretrained model not found at {gen_path}, skipping loading.")
            return
        ...
        try:
            ...
            if self.phase == "train" and os.path.exists(opt_path):
